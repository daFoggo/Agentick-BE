...
class TeamMemberService(BaseService):
    ...
    def add_member(self, team_id: str, schema: TeamMemberCreate, current_user: User):
        ...
        existing_member = self._repository.read_by_options(
            TeamMemberFind(team_id__eq=team_id, user_id__eq=schema.user_id)
        )
    ...
    def generate_invite_token(
        self, team_id: str, email: str, role: str, current_user: User
    ) -> str:
        # Check team exists
        team = self._team_repository.read_by_id(team_id)
        if not team or team.is_deleted:
    ...
    def accept_invite_token(self, token: str, current_user: User):
        ...
        if (
            ...
            or decoded.get("invite_type") != "team"
        ):
    ...
    def update_member_role(
        self, team_id: str, user_id: str, schema: TeamMemberUpdate, current_user: User
    ):
        # Check permission
        current_member = self._repository.read_by_options(
            TeamMemberFind(team_id__eq=team_id, user_id__eq=current_user.id)
    ...
    def check_permission(
        ...
        member_result = self._repository.read_by_options(
            TeamMemberFind(team_id__eq=team_id, user_id__eq=user_id)
        )
