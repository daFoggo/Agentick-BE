...
class InvitationService:
    ...
    def get_my_pending_invitations(self, current_user: User) -> list[Invitation]:
        return self.invitation_repository.get_pending_by_email(
            current_user.email, eager=True
        )
    ...
    def accept_invitation(self, invitation_id: str, current_user: User) -> Invitation:
        ...
        if invitation.team_id:
            ...
            existing = self.team_member_repository.read_by_options(
                TeamMemberFind(
                    team_id__eq=invitation.team_id, user_id__eq=current_user.id
                )
            )
        ...
        if invitation.project_id:
            ...
            existing_project_member = self.project_member_repository.read_by_options(
                ProjectMemberFind(
                    project_id__eq=invitation.project_id, user_id__eq=current_user.id
                )
            )
        ...
        updated_invitation = self.invitation_repository.update(
            invitation_id, {"status": InvitationStatus.ACCEPTED}, eager=True
        )
    ...
    def decline_invitation(self, invitation_id: str, current_user: User) -> Invitation:
        ...
        if invitation.email.strip().lower() != current_user.email.strip().lower():
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="You are not allowed to decline this invitation",
            )

        updated_invitation = self.invitation_repository.update(
            invitation_id, {"status": InvitationStatus.DECLINED}, eager=True
        )
        return updated_invitation
