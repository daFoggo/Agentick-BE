...
def get_my_notifications(
    ...
    notifications = service.get_user_notifications(
        current_user.id, status, is_read, is_bookmarked
    )
    return ResponseSchema(
        data=notifications["founds"], message="Notifications fetched successfully"
    )


@router.patch(
    "/{notification_id}/bookmark", response_model=ResponseSchema[NotificationResponse]
)
def toggle_bookmark(
    notification_id: str,
    current_user: User = Depends(get_current_active_user),
    ...
    return ResponseSchema(
        data=notification, message="Notification bookmark status updated"
    )
...
@router.patch(
    "/{notification_id}/read", response_model=ResponseSchema[NotificationResponse]
)
def mark_as_read(
    notification_id: str,
    current_user: User = Depends(get_current_active_user),
...
@router.patch(
    "/{notification_id}/unarchive", response_model=ResponseSchema[NotificationResponse]
)
def unarchive_notification(
    ...
    return ResponseSchema(
        data=notification, message="Notification restored from archive"
    )
