    ...
    with db_conn.session() as db:
        ...
        try:
            ...
            for r in results[:3]:  # Show first 3
                print(
                    f"  - Task {str(r.get('task_id'))[:8]}... Result: {r.get('risk_score', 'Error')}"
                )
        ...
        try:
            ...
            print(
                f"SUCCESS: Outreach cycle complete. personalized emails sent: {len(outreaches)}"
            )
            for o in outreaches:
                print(
                    f"  - Sent to {o['email']} because of '{o['outreach_type']}' condition."
                )
        except Exception as e:
        ...
        try:
            ...
            print(
                f"SUCCESS: Evening Summary report loop complete. Dispatches: {len(reports)}"
            )
            for rep in reports:
                print(
                    f"  - Project {str(rep['project_id'])[:8]} Status: {rep['status']}"
                )
        except Exception as e:
